import {EventHandlerUtil, DataUtil, getUniqueIdWithPrefix, getCSS} from '../../utils/index'
...
const defaultImageInputQueires: IImageInputQueries = {
  ...
  instanseQuery: '[data-xgn-image-input]',
  inputQuery: 'input[type="file"]',
  wrapperQuery: '.image-input-wrapper',
  cancelQuery: '[data-xgn-image-input-action="cancel"]',
  removeQuery: '[data-xgn-image-input-action="remove"]',
...
class ImageInputComponent {
  ...
  private _change = (e: Event) => {
    ...
    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      // Fire change event
      if (EventHandlerUtil.trigger(this.element, 'xgn.imageinput.change', e) === false) {
        return
      }
      ...
      // Fire removed event
      EventHandlerUtil.trigger(this.element, 'xgn.imageinput.changed', e)
    }
  }
  ...
  private _cancel = (e: Event) => {
    ...
    // Fire cancel event
    if (EventHandlerUtil.trigger(this.element, 'xgn.imageinput.cancel', e) === false) {
      return
    }
    ...
    // Fire canceled event
    EventHandlerUtil.trigger(this.element, 'xgn.imageinput.canceled', e)
  }
  ...
  private _remove = (e: Event) => {
    ...
    // Fire remove event
    if (EventHandlerUtil.trigger(this.element, 'xgn.imageinput.remove', e) === false) {
      return
    }
    ...
    // Fire removed event
    EventHandlerUtil.trigger(this.element, 'xgn.imageinput.removed', e)
  }
