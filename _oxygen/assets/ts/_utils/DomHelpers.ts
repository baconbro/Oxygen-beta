...
function getElementActualCss(el: HTMLElement, prop: any, cache: boolean) {
  ...
  if (!el.getAttribute('xgn-hidden-' + prop) || cache === false) {
    let value
    ...
    // store it in cache
    if (value !== undefined) {
      el.setAttribute('xgn-hidden-' + prop, value.toString())
      return parseFloat(value.toString())
    }
    return 0
  } else {
    // store it in cache
    const attributeValue = el.getAttribute('xgn-hidden-' + prop)
...
function getBreakpoint(breakpoint: string) {
  let value: number | string = getCSSVariableValue('--xgn-' + breakpoint)
  if (value) {
