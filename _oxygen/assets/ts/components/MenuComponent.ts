...
class MenuComponent {
  ...
  constructor(_element: HTMLElement, options: MenuOptions) {
    ...
    // this.element.setAttribute('data-xgn-menu', 'true')
  ...
  private _setTriggerElement = () => {
    const target = document.querySelector(
      `[data-xgn-menu-target="#${this.element.getAttribute('id')}"`
    )

    if (target) {
      this.triggerElement = target as HTMLElement
    } else if (this.element.closest('[data-xgn-menu-trigger]')) {
      this.triggerElement = this.element.closest('[data-xgn-menu-trigger]') as HTMLElement
    } else if (
      this.element.parentNode &&
      getElementChild(this.element.parentNode as HTMLElement, '[data-xgn-menu-trigger]')
    ) {
      const child = getElementChild(
        this.element.parentNode as HTMLElement,
        '[data-xgn-menu-trigger]'
      )
      if (child) {
  ...
  private _getItemOption = (item: HTMLElement, name: string) => {
    ...
    if (item && item.hasAttribute('data-xgn-menu-' + name)) {
      const attr = item.getAttribute('data-xgn-menu-' + name) || ''
  ...
  private _getItemElement = (_element: HTMLElement): HTMLElement | undefined => {
    ...
    if (_element.hasAttribute('data-xgn-menu-trigger')) {
      return _element
    }
    ...
    const item = _element.closest<HTMLElement>('.menu-item[data-xgn-menu-trigger]')
    if (item) {
      return item
    }
  ...
  private _getItemParentElement = (item: HTMLElement) => {
    ...
    const parentItem = sub.closest<HTMLElement>('.menu-item[data-xgn-menu-trigger]')
    if (sub && parentItem) {
      return parentItem
    }
  ...
  private _getItemChildElement = (item: HTMLElement): HTMLElement | null => {
    ...
    if (selector) {
      //element = selector.querySelector('.show.menu-item[data-xgn-menu-trigger]');
      const element = selector.querySelector<HTMLElement>('.menu-item[data-xgn-menu-trigger]')
      if (element) {
        return element
      }
    }
  ...
  private _hasItemSub = (item: HTMLElement) => {
    return item.classList.contains('menu-item') && item.hasAttribute('data-xgn-menu-trigger')
  }
  ...
  private _showDropdown = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'xgn.menu.dropdown.show') === false) {
      return
    }
    ...
    EventHandlerUtil.trigger(this.element, 'xgn.menu.dropdown.shown')
  }
  ...
  private _hideDropdown = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'xgn.menu.dropdown.hide') === false) {
      return
    }
    ...
    EventHandlerUtil.trigger(this.element, 'xgn.menu.dropdown.hidden')
  }
  ...
  private _showAccordion = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'xgn.menu.accordion.show') === false) {
      return
    }
    ...
    if (subElement) {
      ...
      slideDown(sub, this.options.accordion.slideSpeed, () => {
        ...
        item.classList.add('show')
        sub.classList.add('show')
        EventHandlerUtil.trigger(this.element, 'xgn.menu.accordion.shown')
      })
    }
  }

  private _hideAccordion = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'xgn.menu.accordion.hide') === false) {
      return
    }
    ...
    if (sub) {
      slideUp(sub, this.options.accordion.slideSpeed, () => {
        ...
        EventHandlerUtil.trigger(this.element, 'xgn.menu.accordion.hidden')
      })
    }
  }
  ...
  private _hideAccordions = (item: HTMLElement) => {
    const itemsToHide = this.element.querySelectorAll('.show[data-xgn-menu-trigger]')
  ...
  private _update = () => {
    const items = this.element.querySelectorAll('.menu-item[data-xgn-menu-trigger]')
  ...
  private _link = (element: HTMLElement, e: Event) => {
    if (EventHandlerUtil.trigger(this.element, 'xgn.menu.link.click') === false) {
      return
    }

    // Dismiss all shown dropdowns
    MenuComponent.hideDropdowns(undefined)
    EventHandlerUtil.trigger(this.element, 'xgn.menu.link.clicked')
  }
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    const items = document.querySelectorAll<HTMLElement>(
      '.show.menu-dropdown[data-xgn-menu-trigger]'
    )
  ...
  public static updateDropdowns = () => {
    const items = document.querySelectorAll('.show.menu-dropdown[data-xgn-menu-trigger]')
    if (items && items.length > 0) {
  ...
  public static initGlobalHandlers = () => {
    ...
    document.addEventListener('click', (e) => {
      const menuItems = document.querySelectorAll('.show.menu-dropdown[data-xgn-menu-trigger]')
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '.menu-item[data-xgn-menu-trigger] > .menu-link, [data-xgn-menu-trigger]:not(.menu-item):not([data-xgn-menu-trigger="auto"])',
      'click',
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '.menu-item:not([data-xgn-menu-trigger]) > .menu-link',
      'click',
      function (this: HTMLElement, e: Event) {
        e.stopPropagation()
    ...
    DOMEventHandlerUtil.on(
      document.body,
      '[data-xgn-menu-trigger], .menu-sub',
      'mouseover',
      function (this: HTMLElement, e: Event) {
        const menu = MenuComponent.getInstance(this) as MenuComponent
    ...
    window.addEventListener('resize', () => {
      ...
      throttle(
        ...
        () => {
          ...
          const elements = document.querySelectorAll('[data-xgn-menu="true"]')
          elements.forEach((el) => {
  ...
  public static bootstrap = () => {
    ...
    MenuComponent.createInstances('[data-xgn-menu="true"]')
  }
