...
console.log('update',updatedFields)
  ...
  const [movedStatus] = newIssueStatus.splice(source.index, 1);
  newIssueStatus.splice(destination.index, 0, movedStatus);
  ...
  return (
    ...
    {Object.keys(swimlaneGroups).sort().reverse().map((swimlaneTitle, index) => (
      <div key={index} className="my-4">
        {filters.groupBy === 'priority' && (
          <h2>
            <IssuePriorityIcon priority={swimlaneTitle} right={5} />
            {swimlaneTitle}
          </h2>
        )}
        {filters.groupBy === 'type' && (
          <h2>
            <IssueTypeIcon type={swimlaneTitle} />
            {swimlaneTitle}
          </h2>
        )}
        <DragDropContext onDragEnd={handleColumnDragEnd}>
          <Droppable droppableId="all-columns" direction="horizontal" type="column">
            {(provided) => (
              <div style={containerStyle} ref={provided.innerRef} {...provided.droppableProps}>

                {issueStatus.map((status, columnIndex) => (

                        status.id !== 'backlog' ? (
                          <List
                            key={status.id}
                            status={status.id}
                            project={project}//getSwimlaneGroups(swimlaneTitle)}
                            filters={filters}
    ...
    {/* ...remaining code, such as the rendering for empty projects... */}
